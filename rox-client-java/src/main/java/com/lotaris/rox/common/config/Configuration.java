...
/**
 * Configuration for ROX clients. Define general configuration
...
public class Configuration {
	 ...
	 */
	private static final String DEFAULT_HOMEDIR = System.getProperty("user.home") + "/.rox";
	 ...
	 */
	private static final String BASE_CONFIG_PATH = ".rox/config.yml";
	
	/**
	 * Configuration placed in the classpath
	 */
	private static final String CLASSPATH_CONFIG = "rox.yml";
	 ...
	 */
	protected static final String P_ROOT_NODE_NAME = "rox";
	
	/**
	 * Parameter names
	 */
	private static final String P_ROX_WORKSPACE					= P_ROOT_NODE_NAME + ".workspace";
	private static final String P_ROX_SERVER					= P_ROOT_NODE_NAME + ".server";
	private static final String P_ROX_TAGS						= P_ROOT_NODE_NAME + ".tags";
	private static final String P_ROX_TICKETS					= P_ROOT_NODE_NAME + ".tickets";
	private static final String P_ROX_CATEGORY					= P_ROOT_NODE_NAME + ".category";
	private static final String P_ROX_GROUP						= P_ROOT_NODE_NAME + ".group";
	private static final String P_ROX_PUBLISH					= P_ROOT_NODE_NAME + ".publish";
	private static final String P_ROX_GENERATORSEED				= P_ROOT_NODE_NAME + ".seed";

	private static final String P_ROX_PAYLOAD_PRINT				= P_ROOT_NODE_NAME + ".payload.print";
	private static final String P_ROX_PAYLAOD_CACHE				= P_ROOT_NODE_NAME + ".payload.cache";
	private static final String P_ROX_PAYLOAD_SAVE				= P_ROOT_NODE_NAME + ".payload.save";

	private static final String P_ROX_OPTIMIZER_CLASS			= P_ROOT_NODE_NAME + ".java.optimizer.storeClass";
	private static final String P_ROX_OPTIMIZER_CACHE_DIR		= P_ROOT_NODE_NAME + ".java.optimizer.cacheDir";
	private static final String P_ROX_SERIALIZER_CLASS			= P_ROOT_NODE_NAME + ".java.serializerClass";
	...
	protected Configuration() {
		...
		if (config.getNumberOfConfigurations() == 0) {
			...
			disabled = true;
			LOGGER.warn("No server is defined in the ROX configuration files"
					+ "; define servers under the \"servers\" property.");
		} else if (server == null) {
			disabled = true;
			LOGGER.warn("No known server is selected in the ROX configuration files"
					+ "; set the \"server\" property to the name of one of the configured servers.");
		} else if (!server.isValid()) {
			disabled = true;
			LOGGER.warn("The selected server ({}) in the ROX configuration file is invalid", server.getName());
		}
		...
		if (uidDirectory.exists() && uidDirectory.isFile()) {
			throw new IllegalArgumentException("The UID file in the ROX home directory is not a directory");
		}
	...
	private String getMandatory(String name) {
		if (!config.containsKey(name)) {
			throw new RoxConfigurationException(name + " parameter is missing.");
		}
		else {
	...
	/**
	 * @return In case of missing mandatory configuration, ROX will be disabled to allow a smooth failure
	 */
	...
	/**
	 * @return The home directory where ROX client working files are stored
	 */
	public final String getWorkspace() {
		return config.getString(P_ROX_WORKSPACE, DEFAULT_HOMEDIR).replace("~",System.getProperty("user.home"));
	}
	...
	public String getSerializer() {
		return config.getString(P_ROX_SERIALIZER_CLASS);
	}
	...
	private ServerConfiguration getInternalServerConfiguration() {
		return serverList.get(config.getString(P_ROX_SERVER));
	}
	
	private String getServerListDescription() {

		final StringBuilder builder = new StringBuilder("ROX Center server list: ");
		for (final String serverName : serverList.getServerNames()) {
			...
			} else if (serverName.equals(config.getString(P_ROX_SERVER))) {
	...
	/**
	 * @return the ROX API identifier of the project
	...
	public String getProjectApiId() {
		...
		if (globalProjectApiId == null) {
			throw new RoxConfigurationException("ROX project API identifier is missing"
	...
	public String getGroup() {
		...
		return config.getString(P_ROX_GROUP);
	}
	...
	public Long getGeneratorSeed() {
		...
		return config.getLong(P_ROX_GENERATORSEED, null);
	}
	...
	public boolean isPayloadCache() {
		return config.getBoolean(P_ROX_PAYLAOD_CACHE, Boolean.TRUE);
	}
	...
	public boolean isPayloadPrint() {
		return config.getBoolean(P_ROX_PAYLOAD_PRINT, Boolean.FALSE);
	}
	...
	public String getOptimizerStoreClass() {
		return config.getString(P_ROX_OPTIMIZER_CLASS);
	}
	...
	public String getOptimizerCacheDir() {
		return config.getString(P_ROX_OPTIMIZER_CACHE_DIR, DEFAULT_CACHEDIR).replace("~", System.getProperty("user.home"));
	}
	...
	public Set<String> getTags() {
		if (tags == null) {
			...
			List<String> globalTags = (List<String>) config.getProperty(P_ROX_TAGS);
	...
	public Set<String> getTickets() {
		if (tickets == null) {
			...
			List<String> globalTickets = (List<String>) config.getProperty(P_ROX_TICKETS);
	...
	public String getCategory() {
		...
		return config.getString(P_ROX_CATEGORY);
	}
	...
	public boolean isSave() {
		return config.getBoolean(P_ROX_PAYLOAD_SAVE, Boolean.FALSE);
	}

	/**
	 * @return Define if the test results must be send to ROX Server
	 */
	public boolean isPublish() {
		return config.getBoolean(P_ROX_PUBLISH, Boolean.TRUE);
	}
