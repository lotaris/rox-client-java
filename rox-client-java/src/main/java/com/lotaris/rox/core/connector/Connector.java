package com.lotaris.rox.core.connector;
...
import com.lotaris.rox.core.cache.CacheOptimizerStore;
...
/*
 * Connector to send the payloads to ROX
 * 
...
public class Connector {
	 ...
	 */
	public boolean send(RoxPayload payload) throws MalformedURLException {
		...
		LOGGER.info("Connected to ROX Center API at {}", configuration.getServerConfiguration().getApiUrl());
	...
	private URL getPayloadResourceUrl() {
		...
		try {
			...
			LOGGER.error("The selected ROX Center server's API URL is not a valid URL.", ex);
			return null;
		}
		...
		try {
			...
			if (conn.getResponseCode() == 401) {
				LOGGER.error("Authentication to ROX Center failed with API key {}."
						+ " Make sure the API key identifier and shared secret in your ROX configuration file are correct.",
						configuration.getServerConfiguration().getApiKeyId());
				return null;
			}
			...
			try {
				...
				LOGGER.error("The " + API_ROOT_TEST_PAYLOAD_LINK + " link returned by the ROX Center API is not a valid URL", mue);
				return null;
			}
		} catch (IOException ioe) {
			LOGGER.error("Could not read the ROX Center API response", ioe);
			return null;
		}
	}
	 ...
	 */
	private boolean sendPayload(URL payloadResourceUrl, RoxPayload payload, boolean optimized) {
		...
		try {
			...
			if (conn.getResponseCode() == 202) {
				LOGGER.info("The {} was successfully sent to ROX Center.", payloadLogString);
				return true;
			} else {
	 ...
	 */
	private RoxPayload optimize(RoxPayload payload) {
		if (store != null) {
